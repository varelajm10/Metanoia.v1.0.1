...
export class AccountingService {
  ...
  static async getChartOfAccounts(tenantId: string, filters: ChartOfAccountsFilters = {}, page = 1, limit = 10) {
  ...
  static async updateAccount(id: string, data: Partial<ChartOfAccounts>, tenantId: string) {
    const existingAccount = await prisma.chartOfAccounts.findFirst({
      where: { id, tenantId },
    })
  ...
  static async getJournalEntries(tenantId: string, filters: JournalEntryFilters = {}, page = 1, limit = 10) {
    const where: any = { tenantId }
  ...
  static async createJournalEntry(data: JournalEntry, tenantId: string, userId: string) {
  ...
  static async getBankReconciliations(tenantId: string, filters: BankReconciliationFilters = {}, page = 1, limit = 10) {
    const where: any = { tenantId }
    ...
    return { reconciliations, total, page, limit, pages: Math.ceil(total / limit) }
  }

  static async createBankReconciliation(data: BankReconciliation, tenantId: string, userId: string) {
  ...
  static async getTaxes(tenantId: string, filters: TaxFilters = {}, page = 1, limit = 10) {
    const where: any = { tenantId }
  ...
  static async getDashboardStats(tenantId: string) {
    const [
      ...
      prisma.bankReconciliation.count({ where: { tenantId, reconciled: true } }),
