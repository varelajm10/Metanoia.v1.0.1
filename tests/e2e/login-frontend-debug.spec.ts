...
test.describe('Debug Frontend Login', () => {
  test('Diagnóstico completo del flujo de login en el frontend', async ({ page }) => {
    ...
    // Esperar y monitorear la respuesta
    ...
    if (loginResponse) {
      ...
      if (loginResponse.success) {
        ...
        // Esperar un poco más para la redirección
        console.log('⏳ Esperando redirección...')
        await page.waitForTimeout(3000)
        
        const currentUrl = page.url()
        console.log(`🌐 URL actual después de la redirección: ${currentUrl}`)
        
        if (currentUrl.includes('/dashboard/admin')) {
          console.log('✅ Redirección a /dashboard/admin exitosa')
        } else if (currentUrl.includes('/dashboard')) {
          console.log('✅ Redirección a /dashboard exitosa')
    ...
    if (loginResponse && loginResponse.success) {
      // El Super-Admin debería ir a /dashboard/admin
