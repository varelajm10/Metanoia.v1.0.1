...
class AutoCommit {
  ...
  async executeCommit() {
    try {
      ...
      const stats = this.getChangeStats();
      this.log(`📊 Encontrados ${stats.total} cambios: +${stats.added} ~${stats.modified} -${stats.deleted} ?${stats.untracked}`);
      ...
      try {
        ...
        this.log('💡 El commit se guardó localmente, se sincronizará en el próximo push exitoso');
      }
